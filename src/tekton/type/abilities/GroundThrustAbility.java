...
public class GroundThrustAbility extends Ability {
    ...
    public void update(Unit unit) {
        ...
        if (TektonVars.getVec2F(thrusterPosID) == null) {
			TektonVars.addVec2F(new RSeq<>(thrusterPosID));
		}
        ...
        if (reloadProgress >= thrustReload) {
        	if (thrustBulletType != null) {
        		for (Vec2F pos : thrusterPositions) {
	            	...
	            	if (smokeEffect != null && smokeEffect != Fx.none) {
						smokeEffect.at(bulletX, bulletY, pos.rotation + 45f, world.floor((int)(bulletX / 8), (int)(bulletY / 8)).mapColor, unit);
					}
	        	}
	        	if (unit.moving() && unit.speed() > type.speed * mainThrusterMinSpeed) {
	            	...
	            	if (smokeEffect != null && smokeEffect != Fx.none) {
						smokeEffect.at(mainX, mainY, unit.rotation - 180f, world.floor((int)(mainX / 8), (int)(mainY / 8)).mapColor, unit);
					}
	        	}
        	}
        	reloadProgress = 0f;
        ...
        if (unit.moving()) {
        	if (sparkEffect != null && sparkEffect != Fx.none) {
				sparkEffect.at(unit);
			}
        }
