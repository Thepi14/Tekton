...
public class EffectAttackAbility extends Ability {
    ...
    public EffectAttackAbility(float x, float y, Color color, Effect effect, float interval){
        this.x = x;
        this.y = y;
        this.color = color;
        this.effect = effect;
    ...
    public void update(Unit unit){
        ...
        counter += Time.delta;
        if(unit.isShooting && (counter >= interval || (chance > 0 && Mathf.chanceDelta(chance))) && !unit.inFogTo(Vars.player.team())){
            if(rangeLengthMax > 0){
